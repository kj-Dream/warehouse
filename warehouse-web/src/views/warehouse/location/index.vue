...
const edit=(row)=>{isE.value=true;eId.value=row.id;Object.assign(f,{warehouseId:row.warehouseId||whId.value,locationCode:row.locationCode,locationName:row.locationName,status:row.status||1});dw.value=true}
...
const del=async(id)=>{try{await ElMessageBox.confirm('确定删除该库位？','删除确认',{type:'warning'});const r=await deleteLocation(id);if(r.code===200){ElMessage.success('已删除');load()}else ElMessage.error(r.msg)}catch(e){}}
